...
for j in range(len(c_plot)):
    my_data = genfromtxt('ext_time_death_c_%f_fac_10_sc_0.txt' % c_plot[j], delimiter=',')
    data = []
...
high_bc =[]
for j in range(len(c_plot)):
    my_data = genfromtxt('ext_time_death_c_%f_fac_10_sc_1.txt' % c_plot[j], delimiter=',')
    data = []
    for i in range(len(my_data)):
        data.append(float(my_data[i]))
    data = np.asarray(data)
...
plt.plot(c_plot,mean_bc,linestyle='None',marker='^',markersize = '10',color='C1')
