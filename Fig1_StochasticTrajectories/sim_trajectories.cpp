...
// g++ -std=c++11 sim_trajectories.cpp `pkg-config --libs gsl` command for compiling
...
// Main code
double RUN(double,double,int,double,int,int);
...
int main(int argc,char *argv[])
{
    ...
    int repeats = 10;                         // number of samples
    
    while (success<repeats)
    {
        gsl_rng_set(r,r_ind);                     // setting the seed
        time = RUN(c,tfin,scenario,fac,fac2,success);                  // simulation output
...
double RUN(double c,double tfin,int scenario,double fac,int fac2, int ct)
{
    ...
    ofstream file ("trajectory_" + std::to_string(ct) + ".txt");   
    ...
    psiminR = psiminS * 24;                  // minimal growth rate for any antimicrobial concentration resistant
