...
################################
################################ 
c_plot = [0.000100, 0.000200, 0.000400, 0.000600, 0.001000, 0.002000, 0.004000, 0.006000, 0.010000, 0.020000, 0.040000, 0.060000, 0.100000, 0.200000, 0.400000]
mean_bs = []
low_bs = []
high_bs = []
for j in range(len(c_plot)):
    my_data = genfromtxt('popsize_birth_denovo_c_%f_fac_20_sc_0.txt' % c_plot[j], delimiter=',')
    data = []
...
for j in range(len(c_plot2)):
    my_data = genfromtxt('popsize_birth_denovo_c_%f_fac_20_sc_1.txt' % c_plot2[j], delimiter=',')
    data = []
    for i in range(len(my_data)):
        data.append(float(my_data[i][1]))
    data = np.asarray(data)
