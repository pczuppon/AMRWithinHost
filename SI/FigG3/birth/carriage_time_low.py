...
for j in range(len(c_plot)):
    my_data = genfromtxt('ext_time_birth_c_%f_fac_5_sc_0.txt' % c_plot[j], delimiter=',')
    data = []
...
for j in range(len(c_plot2)):
    my_data = genfromtxt('ext_time_birth_c_%f_fac_5_sc_1.txt' % c_plot[j], delimiter=',')
    data = []
    for i in range(len(my_data)):
        data.append(float(my_data[i]))
    data = np.asarray(data)
...
################################ Plot popsizes
################################
plt.semilogx(c[0:len(time_bs)],time_bs,color='C0',linewidth=3)
plt.plot(c[0:len(time_bc)],time_bc,color='C1',linewidth=3)
