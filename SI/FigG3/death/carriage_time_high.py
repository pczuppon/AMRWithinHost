...
for i in range(len(c)):
    ...
    if (xS_bc[i]>1):
    ...
    elif (xS_bc[i] >= 0 and xS_bc[i] < 1 and xR_bc[i]>0):
        ...
        time_bc.append((t1+t2)/rhoS + TT)
    #
    else: # in this case due to numerical inaccurracies the population size xS goes below zero for intermediate concentrations, which is unrealistic, therefore we added the restriction to xS_bc[i]>=0 above
        time_bc.append(0)
...
for j in range(len(c_plot)):
    my_data = genfromtxt('ext_time_death_c_%f_fac_20_sc_0.txt' % c_plot[j], delimiter=',')
    data = []
...
################################ biocidic
c_plot2 = [0.0001, 0.0002, 0.0004, 0.0006, 0.001, 0.002, 0.004, 0.006, 0.01, 0.02, 0.04, 0.06, 0.1, 0.2,0.4]
mean_bc = []
low_bc = []
high_bc =[]
for j in range(len(c_plot2)):
    my_data = genfromtxt('ext_time_death_c_%f_fac_20_sc_1.txt' % c_plot2[j], delimiter=',')
    data = []
    for i in range(len(my_data)):
        data.append(float(my_data[i]))
    data = np.asarray(data)
...
plt.plot(c_plot,mean_bs,linestyle='None',marker='o',markersize = '10',color='C0')
plt.plot(c_plot2,mean_bc,linestyle='None',marker='o',markersize = '10',color='C1')
