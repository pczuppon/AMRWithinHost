...
// g++ -std=c++11 sim_death_size_freq.cpp `pkg-config --libs gsl` command for compiling
...
int RUN(double c,double tfin,int scenario,double fac,int fac2)
{
    ...
    if (R > 0)
    {
        ofstream file ("popsize_freq_death_c_" + std::to_string(c) + "_fac_" + std::to_string(fac2) + "_sc_" + std::to_string(scenario) + ".txt", ios::app);   // file output, population size R
        file << S;
	    file << ",";
        file << R;
        file << "\n";
