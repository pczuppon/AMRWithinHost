...
for j in range(len(c_plot)):
    my_data = genfromtxt('popsize_freq_death_c_%f_fac_10_sc_0.txt' % c_plot[j], delimiter=',')
    data = []
...
for j in range(len(c_plot2)):
    my_data = genfromtxt('popsize_freq_death_c_%f_fac_10_sc_1.txt' % c_plot2[j], delimiter=',')
    data = []
    for i in range(len(my_data)):
        data.append(float(my_data[i][1])/(float(my_data[i][0])+float(my_data[i][1])))
    data = np.asarray(data)
