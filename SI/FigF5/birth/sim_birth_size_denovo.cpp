...
// g++ -std=c++11 sim_birth_size_denovo.cpp `pkg-config --libs gsl` command for compiling
...
#define nu 1./(10.*(double)K)            // mutation rate S -> R 
...
// Main code
int RUN(double,double,int,double,int);
...
int main(int argc,char *argv[])
{
    ...
    while (success < 1000 && r_ind < 1000000)
    {
        r_ind++;
        gsl_rng_set(r,r_ind);
        success += RUN(c,tfin,scenario,fac,fac2);
    }
...
int RUN(double c,double tfin,int scenario,double fac,int fac2)
{
    ...
    while (t <= tfin && R>0)
    {
        ...
        if (scenario == 0)
        {
            rates[0] = max(0.,(bS-aS-cS*((double)S+(double)R)/(double)K)*(double)S);               // birth sensitive
            rates[1] = dS *(double)S;                // death sensitive
            rates[2] = max(0.,(bR-aR-cR*((double)S+(double)R)/(double)K)*(double)R);               // birth resistant
            rates[3] = dR *(double)R;                // death resistant
        }
            
        // Bactericidal scenario
        if (scenario == 1)
        {
            rates[0] = max(0.,(bS-cS*((double)S+(double)R)/(double)K)*(double)S);                    // birth sensitive
            rates[1] = (dS + aS) *(double)S;                // death sensitive
            rates[2] = max(0.,(bR-cR*((double)S+(double)R)/(double)K)*(double)R);                   // birth resistant
            rates[3] = (dR + aR) *(double)R;                // death resistant
        }     
    ...
    if (R > 0)
    {
        ofstream file ("popsize_birth_denovo_c_" + std::to_string(c) + "_fac_" + std::to_string(fac2) + "_sc_" + std::to_string(scenario) + ".txt", ios::app);   // file output, population size R
        file << c;
	    file << ",";
        file << R;
        file << "\n";
