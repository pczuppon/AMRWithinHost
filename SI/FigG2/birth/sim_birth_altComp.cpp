...
// g++ -std=c++11 sim_birth_altComp.cpp `pkg-config --libs gsl` command for compiling
...
int main(int argc,char *argv[])
{
    ...
    ofstream file ("surv_birth_vary_c_factor_" + std::to_string((int)fac) + "_sc_" + std::to_string(scenario) + ".txt", ios::app);   // file output, average
    file << c;
    file << ",";
    file << avg;  
    file << "\n";
...
int RUN(double c,double tfin,int scenario, double fac)
{
    ...
    double gamma_alt = cS/bS; // reparameteterize competition parameter to fit with original model
    ...
    if (tfin == 0)
    {
        while (R>0 && S>0)
        {
            ...
            if (scenario == 0)
            {
                rates[0] = max(0.,(bS-aS)*(1.-gamma_alt*((double)S+(double)R)/(double)K)*(double)S);        // birth sensitive
                rates[1] = dS *(double)S;                // death sensitive
                rates[2] = max(0.,(bR-aR)*(1.-gamma_alt*((double)S+(double)R)/(double)K)*(double)R);        // birth resistant
            ...
            if (scenario == 1)
            {
                rates[0] = max(0.,(bS*(1.-gamma_alt*((double)S+(double)R)/(double)K)*(double)S));              // birth sensitive
                rates[1] = (dS + aS) *(double)S;                // death sensitive
                rates[2] = max(0.,(bR*(1.-gamma_alt*((double)S+(double)R)/(double)K)*(double)R));              // birth resistant
