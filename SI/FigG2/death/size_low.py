...
for j in range(len(c_plot)):
    my_data = genfromtxt('popsize_death_c_%f_fac_5_sc_0.txt' % c_plot[j], delimiter=',')
    data = []
...
c_plot2 = [0.0001, 0.0002, 0.0004, 0.0006, 0.001, 0.002, 0.004, 0.006, 0.01, 0.02, 0.04, 0.06]
mean_bc = []
low_bc = []
high_bc = []
for j in range(len(c_plot2)):
    my_data = genfromtxt('popsize_death_c_%f_fac_5_sc_1.txt' % c_plot2[j], delimiter=',')
    data = []
    for i in range(len(my_data)):
        data.append(float(my_data[i][1]))
    data = np.asarray(data)
...
plt.semilogx(c[0:len(size_bc)],size_bc,color='C1',linewidth=3)
plt.plot(c,size_bs_det,color='C0',linewidth=3,linestyle='dotted',alpha=.5)
plt.plot(c[0:len(size_bc_det)],size_bc_det,color='C1',linewidth=3,linestyle='dotted',alpha=.5)
plt.semilogx(c_plot,mean_bs,linestyle='None',marker='o',markersize = '10',color='C0')
plt.plot(c_plot2,mean_bc,linestyle='None',marker='o',markersize = '10',color='C1')
plt.vlines(c[170],0,11000,linestyle='dashed',color='black',lw=2)
