...
// g++ -std=c++11 sim_birth_size.cpp `pkg-config --libs gsl` command for compiling
...
int main(int argc,char *argv[])
{
    ...
    while (success < 1000 && r_ind < 1000000)
    {
        r_ind++;
        gsl_rng_set(r,r_ind);
        success += RUN(c,tfin,scenario,fac);
    }
...
int RUN(double c,double tfin,int scenario,double fac)
{
    ...
    if (R > 0)
    {
        ofstream file ("popsize_birth_c_" + std::to_string(c) + "_fac_" + std::to_string((int)fac) + "_sc_" + std::to_string(scenario) + ".txt", ios::app);   // file output, population size R
        file << c;
	    file << ",";
        file << R;
        file << "\n";
